                            ...
                            <div class="canvasBox" ref="divBox" @click="deleteLine($event)">
                                ...
                                <div class="Classification" ref="divs" v-for="item,key,i in allBox" :id="key">
                                    ...
                                    <div class="portBox" v-for="(c, x) in item.value" :key="x" ref="portBox">
                                        <div :id="c.portId" class="ports" @mousedown="mousedown($event,c.portId,key)" @mouseup="mouseup($event,c.portId,key)":ref="c.portId" @click.stop="clickPort($event,key,c.portId,c.name)">
                                            <i class="el-icon-circle-close-outline closes" color="red" @click.stop="deletePort($event,c.portId,key)"></i>
                                            <p  class="nameIn">{{c.name}}</p>
                                            <!-- <input type="text" :value="c.name"> -->
                                        </div>
                                    </div>
                                    <div class="add" @click.stop="clickAdd(key)">
                                        <h1>+</h1>
                                    </div>
                                </div>
...
export default class Home extends Vue {
  ...
  created() {
    // console.log(this);
    this.getProtRelFun().then((res: any) => {
        ...
        for (let x in allBox) {
          if (i === x) {
            ...
            if (list[i].value.length > 0) {
              // console.log(this.allBox[i].name, list[i].name);
              // this.allBox[x].value = list[i].value;
              // this.allBox[i].name = list[i].name;
            }
          }
        }
      }
  ...
  mounted() {
    ...
    this.nowPortFun(this.allBox["upLoad"].value[0].portId); //当前选择的端口ID
  ...
  initialize() {
    ...
    console.log(this.lineList, "----初始化端口信息----");
  ...
  clearCanvas() {
    ...
    console.log("--------------已清除-------------");
    this.ctx.clearRect(0, 0, this.divBox.offsetWidth, this.divBox.offsetHeight);
  }
  ...
  getDivLocation(divBox: any, odiv: any) {
    ...
    let left =
      odiv.getBoundingClientRect().left - divBox.getBoundingClientRect().left;
    let top =
      odiv.getBoundingClientRect().top - divBox.getBoundingClientRect().top;
    return {
      x: left,
      y: top
    };
  ...
  clickAdd(key: any) {
    ...
    for (let i in allBox) {
      ...
      if (key === i) {
        ...
        allBox[i].value.push(addData);
      }
    }
    // this.oldLineList = this.lineList.slice(0);
  }
  deletePort(e: any, id: any, key: any) {
